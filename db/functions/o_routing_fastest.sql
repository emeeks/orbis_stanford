CREATE OR REPLACE FUNCTION o_routing_fastest(IN source integer, IN target integer, IN month integer, IN vehicle character varying, IN mode character varying)
...
source::integer,
target::integer,
(o_speed('''||$4||''',type,the_geom,cost) + o_alt_adjust('''||$4||''',restricted)) as cost
FROM
...
source,
target,
(o_speed('''||$4||''',type,the_geom,cost) + o_alt_adjust('''||$4||''',restricted))
',$1,$2,true,false)
